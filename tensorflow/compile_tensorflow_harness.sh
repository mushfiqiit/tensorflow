...
TENSOR_LIST_OPS_CC="$ROOT/tensorflow/compiler/tf2xla/kernels/tensor_list_ops.cc"
TENSOR_LIST_OPS_BC="$OUTDIR/tensor_list_ops.bc"
...
TENSOR_SHAPE_CC="$ROOT/tensorflow/core/framework/tensor_shape.cc"
TENSOR_SHAPE_BC="$OUTDIR/tensor_shape.bc"
...
compile_all() {
  ...
  [[ -f "$HARNESS_BC" ]] || { echo "❌ Expected $HARNESS_BC but it was not created"; return 1; }
    ...
    echo "🛠️  Compiling tensor_list_ops.cc -> $TENSOR_LIST_OPS_BC"
  "$CLANG" -emit-llvm -c "${CXXFLAGS[@]}" "${INCLUDES[@]}" "$TENSOR_LIST_OPS_CC" -o "$TENSOR_LIST_OPS_BC" 2>> clang.err
  if [[ ! -f "$TENSOR_LIST_OPS_BC" ]]; then
    echo "❌ Expected $TENSOR_LIST_OPS_BC but it was not created"
    set -e; return 1
  fi

  echo "🛠️  Compiling tensor_shape.cc -> $TENSOR_SHAPE_BC"
  "$CLANG" -emit-llvm -c "${CXXFLAGS[@]}" "${INCLUDES[@]}" "$TENSOR_SHAPE_CC" -o "$TENSOR_SHAPE_BC" 2>> clang.err
  if [[ ! -f "$TENSOR_SHAPE_BC" ]]; then
    echo "❌ Expected $TENSOR_SHAPE_BC but it was not created"
    set -e; return 1
  fi
