...
compile_all() {
  ...
  echo "🛠️  Compiling tensor.cc -> $TENSOR_BC"
  "$CLANG" -emit-llvm -c "${CXXFLAGS[@]}" "${INCLUDES[@]}" "$TENSOR_CC" -o "$TENSOR_BC" 2>> clang.err
  if [[ ! -f "$TENSOR_BC" ]]; then
    echo "❌ Expected $TENSOR_BC but it was not created"
    set -e; return 1
  fi
...
link_all() {
  ...
  "$LLVMLINK" "$HARNESS_BC" "$TENSOR_SHAPE_BC" "$TENSOR_BC" "${EXTRA_BCS[@]}" -o "$OUT"
}
