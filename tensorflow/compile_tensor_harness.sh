...
compile_all() {
  ...
  echo "🛠️  Compiling tensor_array_ops.cc -> $TENSOR_ARRAY_OPS_BC"
  "$CLANG" -emit-llvm -c "${CXXFLAGS[@]}" "${INCLUDES[@]}" "$TENSOR_ARRAY_OPS_CC" -o "$TENSOR_ARRAY_OPS_BC" 2>> clang.err
  if [[ ! -f "$TENSOR_ARRAY_OPS_BC" ]]; then
    echo "❌ Expected $TENSOR_ARRAY_OPS_BC but it was not created"
    set -e; return 1
  fi
  ...
  echo "🛠️  Compiling status.cc -> $STATUS_BC"
...
link_all() {
  ...
  "$LLVMLINK" "$HARNESS_BC" "$TENSOR_ARRAY_OPS_BC" "$OP_KERNEL_BC" "$STATUS_BC" "${EXTRA_BCS[@]}" -o "$OUT"
}
